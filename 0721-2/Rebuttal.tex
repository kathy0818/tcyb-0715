...
% 本轮(0721 导师意见)颜色标记 —— 详见随附 README_颜色说明.txt
%%   黄底 \FILLIN{}      = 学生必须用原始数据/正式脚本计算或提供
...
and we are grateful to the Associate Editor and Reviewer~1 for the remaining suggestions. \edt{These suggestions concern visualization representativeness, module presentation, failure analysis, and positioning relative to recent work.}

In this second-round revision, \edt{we have addressed the remaining comments as detailed below.} Specifically, we have
(i) clarified the selection criteria of the qualitative visualizations and added exact configuration labels and
\edt{per-image AIU values} to the caption of the frequency-ablation visualization (Fig.~6);
...
(iii) added a new subsection ``Failure Cases and Limitations'' (Section~IV-H) that presents \edt{illustrative} failure
...
\edt{We have addressed the remaining concerns as detailed below:}
...
\begin{itemize}
    ...
    visualization in Fig.~6, label the exact configuration of each panel and report its \edt{per-image AIU} in the caption.
    ...
    \edt{illustrative} failure examples grouped into two observed failure modes, analyzes why FANet struggles in these
...
contributions, and the constructive suggestions. \edt{We address the three remaining comments as detailed below.}\\[10pt]
...
\begin{enumerate}
    ...
    \item \textbf{\edt{Per-image metrics added to the caption.}}
    \edt{The per-image AIU of each visualized prediction is now reported in the caption of Fig.~6, computed with the same
...
selected illustrative case from the Crack500 test set (sample ID \FILLIN{Crack500 sample ID}), rather than a random or best-performing sample.
...
\emph{Baseline + FAM (Low-Freq)} in Table~II, respectively; \edt{the caption reports a per-image AIU for each panel, computed with the same evaluation code as Table~II over the threshold set} \FILLIN{AIU threshold set + step}.
}
...
\textcolor{red}{
The examples in Fig.~5 are purposefully selected \edt{illustrative} cases from the five test sets.
...
\edt{The dataset and sample ID of each row are listed in} \FILLIN{dataset name + sample ID per row of Fig.5}.
}\\[10pt]
...
edge responses acquire contextual support and contour responses are cleansed of texture noise, and (iii) \cck{re-weight and merge the two bands using learnable weights}. The output is a frequency-balanced feature in which crack
...
more precisely. \edt{Multiplying adjacent feature maps promotes responses that are jointly activated across semantic levels, thereby suppressing level-specific noise and producing a cleaner coarse localization map} $S_1$.
}
...
crack-relevant regions of the deeper feature. The second is \emph{what to keep}: \cck{a cross-layer feature interaction between
...
Although FANet achieves \edt{the highest values among the methods compared in Table~I}, it still struggles under two identifiable
conditions. Fig.~7 shows two \edt{illustrative} failure cases selected from the test sets. In each column, the top row is the input
...
limitations are worth noting. First, the two-step coarse-to-fine pipeline \edt{reduces throughput by 22.7\%} (from 13.2 FPS for the PVT
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\linewidth]{failure_cases_v2.pdf}
